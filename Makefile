...
.PHONY: bash-tools
bash-tools:  ## download and bootstrap BuildScale/DevOps-Scripts
	@if ! command -v check_pytools.sh; then \
		curl -L https://git.io/bash-bootstrap | sh; \
	fi

.PHONY: test
test: bash-tools  ## run tests - download BuildScale/DevOps-Scripts and run pytools tests
	@echo "running tests:"
	check_pytools.sh
